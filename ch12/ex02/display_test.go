...
import (
	...
	"github.com/lapis-zero09/5/ch07/lesson10/eval"
)
...
func Example_MapKeyStruct() {
	...
	Display("m", m)
	// Output:
	// Display m (map[display.Item]int):
	// m[display.Item{.Name = "aaa", .Cost = 2000}] = 100
	// m[display.Item{.Name = "bbb", .Cost = 195}] = 2
}

func Example_MapKeyArray() {
	// map struct key
	m := map[[1]int]int{
		[1]int{1}: 100,
		[1]int{2}: 2,
